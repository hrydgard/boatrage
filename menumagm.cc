...
int position=0; //placering nästa båt kommer på
int quitseason=0; //vill spelaren avsluta säsongen?
int wannaquitthis=0; //vill spelaren gå tillbaka?
int inseason=0;   //är 1 om man spelar en säsong
   ...
   if (exists(file)) {
    ...
    for (y=0; y<map->getheight(); y++) {
     for (x=0; x<map->getwidth(); x++) {
      tmp=map->get(x,y,0);
      if (tmp<0 || tmp>255) tmp=0; //för att vara säker
